...
class UsersController extends Controller
{
    ...
    public function admin_store(Request $request)
    {
        ...
        // if (zapier_status_check('user_registration')) {
        //     // Fetch the Zapier setting for 'user_registration'
        //     $zap = ZapierSetting::where('slug', 'user_registration')->where('status', 1)->first();

        //     if ($zap && !empty($zap->webhook_url)) {

        //         $admin_id = User::latest('id')->first();

        //         // Send webhook to Zapier
        //         Http::post($zap->webhook_url, [
        //             'user' => [
        //                 'name' => $admin_id->name,
    ...
    public function student_post(Request $request)
    {
        ...
        for ($i = 0; $i < count($request->user_id); $i++) {
            for ($j = 0; $j < count($request->course_id); $j++) {
                ...
                if ($user) {
                    Enrollment::where('user_id', $data['user_id'])->where('course_id', $data['course_id'])->update(['expiry_date' => $data['expiry_date']]);
                } else {
