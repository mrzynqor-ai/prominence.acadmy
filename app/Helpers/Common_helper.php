...
if (! function_exists('course_by_instructor_frontend')) {
    function course_by_instructor_frontend($course_id = "")
    {
        if ($course_id != '') {
            ...
            if ($course) {
                ...
                if (! empty($course->instructor_ids)) {
                    $decoded_ids = json_decode($course->instructor_ids, true);
                    if (is_array($decoded_ids)) {
                        $instructor_ids = array_merge($instructor_ids, $decoded_ids);
                    }
                }

                // user_id  add
                if (! empty($course->user_id)) {
                    $instructor_ids[] = $course->user_id;
                }

                // duplicate instructor remove
                $instructor_ids = array_unique($instructor_ids);

                if (count($instructor_ids) > 0) {
                    return App\Models\User::whereIn('id', $instructor_ids)->get();
                }
            }
        }

        // course not empty collection
        return collect();
    }
}
            ...
            $count_course = DB::table('courses')
                ...
                ->where(function ($query) use ($user_id) {
                    $query->where('user_id', $user_id)
                        ->orWhereJsonContains('instructor_ids', (string) $user_id);
                })
                ->count();

            return ($count_course > 1)
                ? "{$count_course} " . get_phrase('Courses')
                : "{$count_course} " . get_phrase('Course');
        }
    }
}
...
if (! function_exists('duration_to_seconds')) {
    function duration_to_seconds($duration = "00:00:00:")
    {
        ...
        $hour_to_seconds   = (isset($time_array[0]) && is_numeric($time_array[0]) ? $time_array[0] : 0) * 60 * 60;
        $minute_to_seconds = (isset($time_array[1]) && is_numeric($time_array[1]) ? $time_array[1] : 0) * 60;
        $seconds           = (isset($time_array[2]) && is_numeric($time_array[2]) ? $time_array[2] : 0);
...
if (! function_exists('removeScripts')) {
    function removeScripts($text)
    {
        ...
        $cleanText = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $text);
        $cleanText = preg_replace('#<iframe(.*?)>(.*?)</iframe>#is', '', $text);
...
//ebook
if (! function_exists('user_ebook')) {
    function user_ebooks($user_id = "")
    {
        $count_ebooks = App\Models\Ebook::where('user_id', $user_id)->count();
        return $count_ebooks;
    }
}

if (! function_exists('ebook_review')) {
    function ebook_review($ebook_id = "")
    {
        $count_ebooks = App\Models\EbookReview::where('ebook_id', $ebook_id)->count();
        return $count_ebooks;
    }
}

// instructor rating
if (! function_exists('ebook_instructor_rating')) {
    function ebook_instructor_rating($ebook_id)
    {
        $avg_stars = DB::table('ebook_reviews')
            ->where('ebook_id', $ebook_id)
            ->avg('rating');
        return number_format(intVal($avg_stars), 2);
    }
}
